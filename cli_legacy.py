...
try:
    ...
    print("[WARN] 'pywhatkit' not installed. Music playback will be disabled.")
    pywhatkit = None

# ✅ Gemini API Setup — key must come from .env (copy .env.example -> .env).
# A real key was previously hardcoded here; it has been removed. Rotate/revoke
# that old key on Google AI Studio if it was ever committed to a public repo.
load_dotenv()
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "")
if GEMINI_API_KEY:
    genai.configure(api_key=GEMINI_API_KEY)
else:
    print("[WARN] GEMINI_API_KEY not set. Add it to your .env file — AI replies will fail without it.")
...
# 'msvcrt' (used for keypress-interrupt detection) only exists on Windows.
# Guard it so the CLI still runs on macOS/Linux, just without that one feature.
try:
    import msvcrt
    HAS_MSVCRT = True
except ImportError:
...
def speak(text):
    ...
    for sentence in sentences:
        ...
        # Check for interruption (Keyboard only, Windows-only feature)
...
def ai_response_spec(topic):
    """Strict spec-only variant of ai_response — was called in the 'specs' flow
    below but was never defined, which crashed that entire branch (NameError)."""
    prompt = (
        f"Give ONLY the technical specifications of '{topic}' as a concise "
        f"bullet list (no history, no marketing text, no opinions)."
    )
    ...
    while True:
        ...
        if 'play' in query:
            ...
            if pywhatkit:
                ...
                speak("Music module isn't installed, so I can't play that.")
